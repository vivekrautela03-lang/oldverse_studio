...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Mock login success
    alert("Successfully logged in (mock)!");
  };
    ...
    <div className="min-h-screen flex bg-brand-black text-brand-ivory font-sans">
      ...
      <div className="w-full md:w-[58%] flex items-center justify-center px-6 py-12 bg-brand-black">
        <div className="w-full max-w-md">
          {/* Header */}
          <div className="mb-8">
            <h2 className="font-bebas text-4xl tracking-wider text-brand-ivory">Welcome Back</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Remember & Forgot */}
            <div className="flex items-center justify-between text-xs font-space text-brand-ivory/60">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  className="rounded border-white/10 bg-white/5 text-brand-gold focus:ring-0 cursor-pointer h-4 w-4"
                />
                Remember me
              </label>
              <button
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.99 }}
              className="w-full mt-4 font-space text-xs font-semibold uppercase tracking-wider bg-brand-gold text-brand-black py-3 rounded-lg hover:bg-brand-ivory transition-all shadow-glow"
            >
              Login
            </motion.button>
          </form>
          ...
          <p className="mt-8 text-center text-xs font-space text-brand-ivory/40">
            ...
            <Link href="/signup" className="text-brand-gold hover:text-brand-ivory transition-colors">
              Sign up
            </Link>
          </p>
        </div>
